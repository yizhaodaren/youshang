...
@interface MOLClipMovieWhenEditView() <UICollectionViewDataSource, UICollectionViewDelegate,UICollectionViewDelegateFlowLayout>
...
@implementation MOLClipMovieWhenEditView
...
- (instancetype)initWithMovieURL:(NSURL *)url minDuration:(Float64)minDuration maxDuration:(Float64)maxDuration withCustomH:(CGFloat)height showBottom:(BOOL) isShowBottom{
    ...
    return [self initWithMovieAsset:asset minDuration:minDuration maxDuration:maxDuration withCustomH:height showBottom:isShowBottom];
}
...
- (void)initView {
    ...
    //已经选取的时间
    [self.customView addSubview:self.clipSecondLabel];
...
//
    ...
    [self.customView addSubview:progressBarView];
...
-(CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout *)collectionViewLayout sizeForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (self.collectionImages.count == (indexPath.row + 1)) {
