...
@interface MOLUserCommentCell ()
@property (nonatomic, weak) UIImageView *iconImageView;
@property (nonatomic, weak) UILabel *nameLabel;
@property (nonatomic, weak) UILabel *subNameLabel; // 评论内容
@property (nonatomic, weak) UILabel *introduceLabel; // 评论了你的作品  4分钟前
@property (nonatomic, weak) UIImageView *productionImageView;
@property (nonatomic, weak) UIView *lineView;
@end

@implementation MOLUserCommentCell

- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    self = [super initWithStyle:style reuseIdentifier:reuseIdentifier];
    if (self) {
        [self setupUserCommentCellUI];
...
- (void)setCommentModel:(MOLMsgCommentModel *)commentModel
{
    _commentModel = commentModel;
    
    [self.iconImageView sd_setImageWithURL:[NSURL URLWithString:commentModel.baseUserVO.avatar]];
    self.nameLabel.text = commentModel.baseUserVO.userName;
    self.subNameLabel.text = commentModel.commentContent.length ? commentModel.commentContent : @"评论已删除";
    self.introduceLabel.text = [NSString stringWithFormat:@"%@ %@",commentModel.content,[NSString getCommentMessageTimeWithTimestamp:commentModel.createTime]];
    
    [self.productionImageView sd_setImageWithURL:[NSURL URLWithString:commentModel.avatar]];
}
...
- (void)clickIconImageView
{
    ...
    if ([self.commentModel.baseUserVO.userId isEqualToString:user.userId]) {
        ...
        vc.userId = self.commentModel.baseUserVO.userId;
        [[[MOLGlobalManager shareGlobalManager] global_currentNavigationViewControl] pushViewController:vc animated:YES];
    }
}

#pragma mark - UI
- (void)setupUserCommentCellUI
{
    ...
    subNameLabel.text = @"这里是评论的内容....bulabula....";
    ...
    introduceLabel.text = @"评论了你的作品 4分钟前";
...
- (void)calculatorUserCommentCellFrame
{
...
- (void)layoutSubviews
{
    ...
    [self calculatorUserCommentCellFrame];
}
