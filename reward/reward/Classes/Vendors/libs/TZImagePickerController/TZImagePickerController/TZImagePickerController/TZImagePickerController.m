...
- (void)viewDidLoad {
    ...
    self.view.backgroundColor = HEX_COLOR(0x0E0F1A);
...
- (instancetype)initWithSelectedAssets:(NSMutableArray *)selectedAssets selectedPhotos:(NSMutableArray *)selectedPhotos index:(NSInteger)index isOnlyPreview:(BOOL)isOnlyPreview{
    ...
    if (self) {
        ...
        [self configDefaultSetting];
        previewVc.isOnlyPreview =isOnlyPreview;
...
- (void)configDefaultImageName {
    self.takePictureImageName = @"mine_camera_icon";
    self.photoSelImageName = @"选中";
    self.photoDefImageName = @"未选择";
...
- (id)showAlertWithTitle:(NSString *)title {
    if (iOS8Later) {
        ...
        UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:title message:nil delegate:nil cancelButtonTitle:[NSBundle tz_localizedStringForKey:@"我知道了"] otherButtonTitles:nil, nil];
        [alertView show];
        return alertView;
    }
}
...
- (void)setAllowCrop:(BOOL)allowCrop {
    _allowCrop = _maxImagesCount > 0 ? NO : allowCrop;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    photoPickerVc.model = model;
   // [self.navigationController presentViewController:photoPickerVc animated:YES completion:nil] :photoPickerVc animated:YES];
