...
- (void)configCustomNaviBar {
    ...
    //[_naviBar setBackgroundColor: [UIColor redColor]];
    _backButton = [[UIButton alloc] initWithFrame:CGRectZero];
    [_backButton setImage:[UIImage imageNamedFromMyBundle:@"关闭"] forState:UIControlStateNormal];
    ...
    /////////////////////
    if (!self.isOnlyPreview) {
        _doneButton = [UIButton buttonWithType:UIButtonTypeCustom];
        _doneButton.titleLabel.font = [UIFont systemFontOfSize:16];
        [_doneButton addTarget:self action:@selector(doneButtonClick) forControlEvents:UIControlEventTouchUpInside];
        [_doneButton setTitle:@"完成" forState:UIControlStateNormal];
        [_doneButton setTitle:@"完成" forState:UIControlStateDisabled];
        [_doneButton setTitleColor:HEX_COLOR(0xffffff) forState:UIControlStateNormal];
        [_doneButton setTitleColor:[HEX_COLOR(0xffffff) colorWithAlphaComponent:0.7] forState:UIControlStateDisabled];
...
- (void)viewDidLayoutSubviews {
    ...
    _doneButton.frame = CGRectMake(self.view.tz_width - _doneButton.tz_width - 12, 10 + statusBarHeightInterval, _doneButton.tz_width, 44);
    _numberImageView.frame = CGRectMake(_doneButton.tz_left - 24 - 5, 10 + statusBarHeightInterval+10, 24, 24);
...
//    _numberLabel.frame = _numberImageView.frame;
    _selectButton.frame = CGRectMake(self.view.tz_width - 56, 0, 44, 44);
