...
@interface TZPhotoPickerController ()<UICollectionViewDataSource,UICollectionViewDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate,UIAlertViewDelegate,TapChangeAlbumDelegate> {
...
@property (strong, nonatomic) UIButton *navCenterButton;
@property (strong, nonatomic) TapChangeAlbum *albumView;
@property (strong, nonatomic) AlbumShowView *albumShowView;
...
- (UIImagePickerController *)imagePickerVc {
    if (_imagePickerVc == nil) {
        ...
        UIImage *bgImage = [[UIImage imageNamed:@"home_back_top"] resizableImageWithCapInsets:UIEdgeInsetsMake(0, 0, 0, 0) resizingMode:UIImageResizingModeStretch];
        //    [[UINavigationBar appearance] setBackgroundImage:bgImage forBarMetrics:UIBarMetricsDefault];
        [_imagePickerVc.navigationBar setBackgroundImage:bgImage forBarMetrics:UIBarMetricsDefault];
        _imagePickerVc.navigationBar.shadowImage = [UIImage new];
...
- (UIView *)navBgView{
    if (!_navBgView) {
        ...
        [_navBgView setBackgroundColor:[UIColor whiteColor]];
    }
    return _navBgView;
}
...
//        [_navLeftButton setImage:[UIImage imageNamed:@"黑色关闭"] forState:UIControlStateNormal];
        [_navLeftButton setTitle:@"取消" forState:UIControlStateNormal];
        [_navLeftButton setTitleColor:HEX_COLOR_ALPHA(0x221E1E, 0.5) forState:UIControlStateNormal];
        _navLeftButton.titleLabel.font = MOL_MEDIUM_FONT(14);
        [_navLeftButton addTarget:self action:@selector(navLeftButtonEvent:) forControlEvents:UIControlEventTouchUpInside];
    }
    return _navLeftButton;
}

- (TapChangeAlbum *)albumView{
    if (!_albumView) {
        _albumView =[TapChangeAlbum new];
        [_albumView setDelegate:self];
       // [_albumView setBackgroundColor:[UIColor redColor]];
    }
    return _albumView;
}

#pragma mark-
#pragma mark TapChangeAlbumDelegate
- (void)tapChangeAlbum:(TapChangeAlbum*)view option:(TapChangeAlbumOption)option{
    switch (option) {
        case TapChangeAlbumSelectOption:{
            [self.albumShowView cancelView];
            
            CGFloat height =self.groupArr.count*(56+20)+20;
            if (self.groupArr.count*(56+20)+20>MOL_SCREEN_HEIGHT-64-128) {
                height =MOL_SCREEN_HEIGHT-64-128;
            }
            
            self.albumShowView =[[AlbumShowView alloc] initWithFrame:CGRectMake(0, MOL_StatusBarHeight+44, MOL_SCREEN_WIDTH,height)];
            [self.albumShowView setBackgroundColor: [UIColor redColor]];
            [self.albumShowView setShowTableCell:self.groupArr];
            __weak __typeof(self) weakSelf = self;
            self.albumShowView.selectRowBlock = ^(NSIndexPath *indexPath, TZAlbumModel *model) {
                ...
                if (tzImagePickerVc&&tzImagePickerVc.selectedModels.count>0){
                    [tzImagePickerVc.selectedModels removeAllObjects];
                }
                weakSelf.numberImageView.hidden = tzImagePickerVc.selectedModels.count <= 0;
                weakSelf.doneButton.enabled = tzImagePickerVc.selectedModels.count || tzImagePickerVc.alwaysEnableDoneBtn;
                weakSelf.numberLabel.text = [NSString stringWithFormat:@"%zd",tzImagePickerVc.selectedModels.count];
                weakSelf.numberLabel.hidden = tzImagePickerVc.selectedModels.count <= 0;
                weakSelf.models = (NSMutableArray *)model.models;
                [weakSelf.collectionView reloadData];
                [weakSelf.albumShowView cancelView];
                [weakSelf.albumView initTapChangeAlbumTitleButotn];
            };
            
        }
            break;
            
        case TapChangeAlbumCancelOption:{
            [self.albumShowView cancelView];
        }
            break;
...
#pragma mark-
#pragma mark 取消
- (void)navLeftButtonEvent:(UIButton *)sender{
    __weak __typeof(self) weakSelf = self;
    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        ...
        [[TZImageManager manager] getAllAlbums:imagePickerVc.allowPickingVideo allowPickingImage:imagePickerVc.allowPickingImage needFetchAssets:YES completion:^(NSArray<TZAlbumModel *> *models) {
            dispatch_async(dispatch_get_main_queue(), ^{
                self->_groupArr = [NSMutableArray arrayWithArray:models];
                for (TZAlbumModel *albumModel in self->_groupArr) {
...
- (void)defaultTapChangeAlbum:(NSNotification *)notif{
    [self.albumView initTapChangeAlbumTitleButotn];
}
...
- (void)viewDidLoad {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(defaultTapChangeAlbum:) name:@"AlbumShowView" object:nil];
    
    [self configPhotoGroup];
...
- (void)viewWillLayoutSubviews{
    ...
    [self.navBgView setFrame:CGRectMake(0, MOL_StatusBarHeight, [[UIScreen mainScreen] bounds].size.width, 44)];
    
    UIBezierPath *maskPath = [UIBezierPath bezierPathWithRoundedRect:self.navBgView.bounds byRoundingCorners:UIRectCornerTopLeft | UIRectCornerTopRight cornerRadii:CGSizeMake(12, 12)];
    CAShapeLayer *maskLayer = [[CAShapeLayer alloc] init];
    maskLayer.frame = self.navBgView.bounds;
    maskLayer.path = maskPath.CGPath;
    self.navBgView.layer.mask = maskLayer;
    
    [self.navLeftButton setFrame:CGRectMake(0, 0, 44, 50)];
    
    [self.albumView setFrame:CGRectMake(0, 0, 150, 44)];
    [self.albumView setCenterX: self.navBgView.centerX];
    [self.albumView setNeedsLayout];
...
- (void)configNavToolBar{
    TZImagePickerController *tzImagePickerVc = (TZImagePickerController *)self.navigationController;
    if (!tzImagePickerVc.showSelectBtn) return;
    
    _doneButton = [UIButton buttonWithType:UIButtonTypeCustom];
    _doneButton.titleLabel.font =MOL_MEDIUM_FONT(14);
    [_doneButton addTarget:self action:@selector(doneButtonClick) forControlEvents:UIControlEventTouchUpInside];
    [_doneButton setTitle:@"完成" forState:UIControlStateNormal];
    [_doneButton setTitle:@"完成" forState:UIControlStateDisabled];
    [_doneButton setTitleColor:HEX_COLOR(0x091F38) forState:UIControlStateNormal];
    [_doneButton setTitleColor:[HEX_COLOR(0x091F38) colorWithAlphaComponent:0.7] forState:UIControlStateDisabled];
    ...
    [_numberImageView setBackgroundColor: HEX_COLOR(0x2A91D4)];
    ...
    [self.navBgView addSubview:self.doneButton];
    
    
    [self.navBgView addSubview:self.navLeftButton];
    [self.navBgView addSubview:self.albumView];
    [self.navBgView addSubview:_doneButton];
    [self.navBgView addSubview:_numberImageView];
    [self.navBgView addSubview:_numberLabel];
...
- (void)viewDidLayoutSubviews {
    ...
    CGFloat toolBarHeight = [TZCommonTools tz_isIPhoneX] ? (83 - 49) : 0;
    ...
    [_numberImageView.layer setCornerRadius: 24/2.0];
    ...
    if (tzImagePickerVc.autoDismiss) {
        ...
        [UIView animateWithDuration:0.2 animations:^{
            ...
            [weakSelf.navigationController dismissViewControllerAnimated:YES completion:^{
                [weakSelf callDelegateMethodWithPhotos:photos assets:assets infoArr:infoArr];
            }];
        }];
    } else {
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (((tzImagePickerVc.sortAscendingByModificationDate && indexPath.item >= _models.count) || (!tzImagePickerVc.sortAscendingByModificationDate && indexPath.item == 0)) && _showTakePhotoBtn) {
        ...
        if ([tzImagePickerVc.takePictureImageName isEqualToString:@"拍照新"]) {
    ...
    cell.didSelectPhotoBlock = ^(BOOL isSelected) {
        ...
        if (isSelected) {
            ...
            if (tzImagePickerVc.selectedModels.count < tzImagePickerVc.maxImagesCount) {
                ...
                NSString *title = [NSString stringWithFormat:[NSBundle tz_localizedStringForKey:@"你最多只能选择 %zd 张照片"], tzImagePickerVc.maxImagesCount];
                [tzImagePickerVc showAlertWithTitle:title];
            }
        }
    };
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (model.type == TZAssetModelMediaTypeVideo && !tzImagePickerVc.allowPickingMultipleVideo) {
        ...
        //[self.navigationController presentViewController:photoPreviewVc animated:YES completion:nil];
