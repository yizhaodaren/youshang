...
- (void)wm_addCachedViewController:(UIViewController *)viewController atIndex:(NSInteger)index {
    ...
    for (id frame in self.childViewFrames) {
        NSLog(@"cache frame ->%@",frame);
    }
    
    for (id obj in self.childViewControllers) {
        NSLog(@"cache obj 0 ->%@ -->%ld",obj,self.childViewControllers.count);
    }
    ...
    for (id obj in self.childViewControllers) {
        NSLog(@"cache obj 1 ->%@   -->%ld",obj,self.childViewControllers.count);
    }
}
