...
- (void)setupUI
{
    ...
    _textField.layer.borderColor = [[UIColor whiteColor] colorWithAlphaComponent:0.6].CGColor;
...
- (void)layoutSubviews
{
  ...
  CGFloat buntW = _height/2 * 3;
    CGFloat dis = 5;
    
    _textField.frame = CGRectMake(buntW + dis, 0, _width - 2*(buntW + dis ), _height);
    
    _increaseBtn.frame = CGRectMake(_width - buntW, 0, buntW, _height);
    
    if (_decreaseHide && _textField.text.floatValue < _minValue) {
        _decreaseBtn.frame = CGRectMake(_width - buntW, 0, buntW, _height);
    } else {
        _decreaseBtn.frame = CGRectMake(0, 0, buntW, _height);
    }
}
